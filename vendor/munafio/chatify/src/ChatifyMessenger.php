...
class ChatifyMessenger
{
    ...
    public function fetchMessage($id){
        ...
        return [
            ...
            'viewType' => ($msg->from_id == get_current_user_id()) ? 'sender' : 'default',
            'seen' => $msg->seen,
        ];
    }
    ...
    public function fetchMessagesQuery($user_id){
        return Message::where('from_id',get_current_user_id())->where('to_id',$user_id)
                    ->orWhere('from_id',$user_id)->where('to_id',get_current_user_id());
    }
    ...
    public function makeSeen($user_id){
        Message::Where('from_id',$user_id)
                ->where('to_id',get_current_user_id())
                ->where('seen',0)
                ->update(['seen' => 1]);
        return 1;
    }
    ...
    public function countUnseenMessages($user_id){
        return Message::where('from_id',$user_id)->where('to_id',get_current_user_id())->where('seen',0)->count();
    }
    ...
    public function inFavorite($user_id){
        return Favorite::where('user_id', get_current_user_id())
    ...
    public function makeInFavorite($user_id, $action){
        if ($action > 0) {
            ...
            $star->user_id = get_current_user_id();
            $star->favorite_id = $user_id;
            $star->save();
            return $star ? true : false;
        }else{
            // UnStar
            $star = Favorite::where('user_id',get_current_user_id())->where('favorite_id',$user_id)->delete();
            return $star ? true : false;
        }
    }

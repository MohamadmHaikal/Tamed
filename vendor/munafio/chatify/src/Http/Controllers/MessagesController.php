...
class MessagesController extends Controller
{
    ...
    public function index($id = null)
    {
        ...
        return view('Chatify::pages.app', [
            'id' => $id ?? 0,
            'type' => $type ?? 'user',
            'messengerColor' => get_current_user_data()->messenger_color ?? $this->messengerFallbackColor,
            'dark_mode' => get_current_user_data()->dark_mode < 1 ? 'light' : 'dark',
        ]);
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('users',  function ($join) {
            ...
            ->where(function ($q) {
                $q->where('ch_messages.from_id', get_current_user_id())
                    ->orWhere('ch_messages.to_id', get_current_user_id());
            })
        ...
        $users = $users->where('id', '!=', get_current_user_id());
        if ($users->count() > 0) {
            // fetch contacts
            $contacts = '';
            foreach ($users as $user) {
                if ($user->id != get_current_user_id()) {
                    ...
                    $contacts .= Chatify::getContactItem($userCollection);
                }
            }
        }
    ...
    {
        $favoritesList = null;
        $favorites = Favorite::where('user_id', get_current_user_id());
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('id', '!=', get_current_user_id())
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? User::where('id', get_current_user_id())->update(['dark_mode' => 1])  // Make Dark
                : User::where('id', get_current_user_id())->update(['dark_mode' => 0]); // Make Light
        }

        // If messenger color selected
        if ($request['messengerColor']) {
            $messenger_color = trim(filter_var($request['messengerColor'], FILTER_SANITIZE_STRING));
            User::where('id', get_current_user_id())
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (get_current_user_data()->avatar != config('chatify.user_avatar.default')) {
                        $path = storage_path('app/public/' . config('chatify.user_avatar.folder') . '/' . get_current_user_data()->avatar);
                        if (file_exists($path)) {
                            @unlink($path);
                        }
                    }
                    ...
                    $update = User::where('id', get_current_user_id())->update(['avatar' => $avatar]);
